        ...
        <div class="loginform">
          ...
          <div class="content">
            <form>
              ...
              <!-- 绑定点击事件，点击时登录，并且取消按钮默认功能，使用prevent修饰符，修click事件 -->
...
export default {
  ...
  methods: {
    // 点击登录时，派发登录action，并传递用户输入的信息
    async getUserLogin() {
      // 获取跳转到query参数，因为有的时候，login地址携带了query参数，需要登录后，跳转到query参数保存到地址上
      // 如果没有query参数则直接跳转到home页面
      let toPath = this.$route.query.redirect || "/home";
      // 校验到用户输入的用户名和密码是否符合标准，全部通过则提交请求
      ...
      if (validate) {
        ...
        try {
          ...
          // 简单是判断一下用户输入的信息必须有，不能为空
          await this.$store.dispatch("getUserLogin", { phone, password });
          // 获取到登录成功的信息后，跳转到home页面
          // 之后的请求就可以携带token，向服务器发起请求了
